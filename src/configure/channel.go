...
func Init() {
	...
	_, err := RoomKeys.redisCli.Ping(context.Background()).Result()
	if err != nil {
		log.Panic("Redis: ", err)
	}
...
func (r *RoomKeysType) SetKey(channel string) (key string, err error) {
	if !saveInLocal {
		for {
			...
			if _, err = r.redisCli.Get(context.Background(),key).Result(); err == redis.Nil {
				err = r.redisCli.Set(context.Background(),channel, key, 0).Err()
				if err != nil {
					return
				}

				err = r.redisCli.Set(context.Background(),key, channel, 0).Err()
				return
			} else if err != nil {
				return
			}
...
func (r *RoomKeysType) GetKey(channel string) (newKey string, err error) {
	if !saveInLocal {
		if newKey, err = r.redisCli.Get(context.Background(),channel).Result(); err == redis.Nil {
...
func (r *RoomKeysType) GetChannel(key string) (channel string, err error) {
	if !saveInLocal {
		return r.redisCli.Get(context.Background(),key).Result()
	}
...
func (r *RoomKeysType) DeleteChannel(channel string) bool {
	if !saveInLocal {
		return r.redisCli.Del(context.Background(),channel).Err() != nil
	}
...
func (r *RoomKeysType) DeleteKey(key string) bool {
	if !saveInLocal {
		return r.redisCli.Del(context.Background(),key).Err() != nil
	}
